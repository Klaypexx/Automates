    ...
    def next_token(self):
        while not self.eof or self.buffer:
            ...
            if match:
                ...
                self.buffer = self.buffer[len(lexeme):]
                self.current_column += len(lexeme)
                    # добавить проверку на разделитель
                self.buffer = self.buffer.replace("\n", "")

                if self.buffer and not re.match(r"[ \t\(\)\+\-\*/;,=\[\]{}<>'.:]", self.buffer[0]) and not re.match(r"^[a-zA-Z0-9_]*$", self.buffer):
                    self.current_column -= len(lexeme)
...
def main():
    ...
    input_file = "Lab6Input.csv"
    output_file = "Lab6Output.csv"
